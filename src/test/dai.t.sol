...
contract DaiTest is DSTest {
    ...
    //*** interesting, no assertion on this one
    ...
    function testValidTransfers() public logs_gas {
        uint sentAmount = 250;
        //*** not sure what the point of emitting this is 
    ...
    //*** simply asserting that the test fails rather than fails with a specific message
    //*** presumably this fails because user2 doesn't have enough tokens and because user2 has not approved the test contract
    ...
    function testChargesAmountApproved() public logs_gas {
        ...
        //*** added assert for verifying that user2's balance increased
        assertEq(token.balanceOf(user2), amountApproved);
