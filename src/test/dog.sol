...
contract DogTest is DSTest {
    ...
    function testSetUrn(uint256 ink, uint256 art) internal {
        vat.slip(ilk, usr, int256(ink));
        assertEq(vat.gem(ilk, usr), ink);
        (, uint256 rate,,,) = vat.ilks(ilk);
        assertEq(rate, 10**27);

        vat.suck(address(vow), address(vow), art * rate);

        assertEq(vat.sin(usr), art*rate);
        assertEq(vat.dai(usr), art*rate);
        assertEq(vat.vice(), art*rate);
        assertEq(vat.debt(), art*rate);

        vat.grab(ilk, usr, usr, address(vow), int256(ink), int256(art));

        assertEq(vat.gem(ilk, usr), 0);
        assertEq(vat.sin(usr), 0);
        assertEq(vat.vice(), 0);
